...
- **لا تشارك** ملف الجلسة (`.session`) أو Session String مع أي شخص
...
| 🖼️ دعم الألبومات | ينقل Albums/MediaGroups كألبوم واحد بدل رسائل منفردة (v2.1) |
| 📱 واجهة سهلة | واجهة Gradio عربية بالكامل |
| 🎛️ تحكم كامل | عدد الرسائل، التأخير، أنواع الوسائط، تصفية النص |
| 🔑 حفظ الجلسة | Session String لتجنّب إعادة تسجيل الدخول (مهم لـ HF Spaces) |
| 📊 إحصائيات مفصّلة | تبويب مخصص مع عدد الألبومات والمنفردة والأخطاء (v2.1) |
...
### 3️⃣ حفظ الجلسة (مهم لـ HuggingFace)
...
4. احفظ `SESSION_STRING` في **Settings → Repository secrets**
5. اضغط **Build** — التطبيق يشتغل تلقائياً على بورت `7860`
...
### دعم الألبومات (v2.1)
التطبيق يكتشف تلقائياً `grouped_id` للرسائل المجمّعة، يجمع كل ملفات الألبوم،
ويرسلها كدفعة واحدة باستخدام `send_file(dest, files, ...)` بدل إرسال كل صورة منفصلة.
